...
import { getValueByPath, escapeRegexpString, isIE, isEdge, valueEquals } from 'element-ui/lib/utils/util.js';
...
import scrollIntoView from 'element-ui/lib/utils/scroll-into-view.js';
...
var script = {
  ...
  methods: {
    ...
    resetInputHeight: function resetInputHeight() {
      ...
      this.$nextTick(function () {
        if (!_this0.$refs.reference) return;
        var inputChildNodes = _this0.$refs.reference.$el.childNodes;
        var input = [].filter.call(inputChildNodes, function (item) {
          return item.tagName === 'INPUT';
        })[0];
        var tags = _this0.$refs.tags;
        var tagsHeight = tags ? Math.round(tags.getBoundingClientRect().height) : 0;
        var sizeInMap = _this0.initialInputHeight || 40;
        input.style.height = _this0.selected.length === 0 ? sizeInMap + 'px' : Math.max(tags ? tagsHeight + (tagsHeight > sizeInMap ? 6 : 0) : 0, sizeInMap) + 'px';
        if (_this0.visible && _this0.emptyText !== false) {
          _this0.broadcast('ElSelectDropdown', 'updatePopper');
        }
      });
    },
    resetHoverIndex: function resetHoverIndex() {
      ...
      setTimeout(function () {
        if (!_this1.multiple) {
          _this1.hoverIndex = _this1.options.indexOf(_this1.selected);
        } else {
          if (_this1.selected.length > 0) {
            _this1.hoverIndex = Math.min.apply(null, _this1.selected.map(function (item) {
              return _this1.options.indexOf(item);
            }));
          } else {
            _this1.hoverIndex = -1;
          }
  ...
  created: function created() {
    ...
    this.debouncedQueryChange = debounce(this.debounce, function (e) {
      _this11.handleQueryChange(e.target.value);
    });
  ...
  mounted: function mounted() {
    ...
    this.$nextTick(function () {
      if (reference && reference.$el) {
        _this12.inputWidth = reference.$el.getBoundingClientRect().width;
      }
    });
    this.setSelected();
  },
