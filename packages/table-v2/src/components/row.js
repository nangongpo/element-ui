...
export default {
  ...
  props: { columns: Array, rowData: Object, rowIndex: Number, rowKey: [String, Number], depth: Number, cellStyles: Array, cellProps: [Object, Function], expandColumnKey: [String, Number], expanded: Boolean, indentSize: Number, iconSize: Number, isScrolling: Boolean, rowClass: [String, Array, Object], rowProps: Object, rowStyle: Object, cellRenderer: [Object, Function], dataGetter: Function, rowSlot: Function },
  methods: {
    cellValue(column) {
    ...
    cellWidth(index) {
      return this.cellStyles.slice(index, index + 1).reduce((total, style) => {
  ...
  render(h) {
    ...
    this.columns.forEach((column, index) => {
      ...
      const expandable = this.expandColumnKey != null && column.key === this.expandColumnKey && this.rowData && Array.isArray(this.rowData.children) && this.rowData.children.length > 0;
    ...
    const content = this.rowSlot ? this.rowSlot({
      cells,
    ...
    const rowDepthClass = this.expandColumnKey && this.rowIndex >= 0 ? `el-table-v2__row-depth-${this.depth || 0}` : null;
    return h('div', { class: ['el-table-v2__row', rowDepthClass, this.rowClass], style: this.rowStyle, on: this.$listeners, attrs: { role: 'row' } }, Array.isArray(content) ? content : [content]);
  }
};
